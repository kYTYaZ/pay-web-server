...
public class OrderServiceImpl implements IOrderService {
     ...
     */
	@Override
	public OutputParam queryOrderMonth(InputParam inputParam) throws FrameException {
		logger.info("查询月表订单信息请求报文:"+inputParam.toString());
		OutputParam queryOrderOut = new OutputParam();
		
		try {
			
			List<Map<String, Object>> orderList = orderDao.queryOrderMonth(inputParam.getParamString());
			if (StringUtil.listIsEmpty(orderList)) {
				queryOrderOut.setReturnCode(StringConstans.returnCode.FAIL);
				queryOrderOut.setReturnMsg("查询无此订单");
				return queryOrderOut;
			...
			logger.error("查询二维码月表订单信息失败" + e.getMessage(), e);
			throw new FrameException(e);
		}
		logger.info("查询月表订单信息返回报文:"+queryOrderOut.toString());
		return  queryOrderOut;
	}
     ...
     */
	@Override
	public OutputParam queryMerEnableMoneyByDay(InputParam inputParam) throws FrameException {
		logger.info("查询商户当天可退款金额报文:"+inputParam.toString());
		OutputParam queryOrderOut = new OutputParam();
		
		try {
			
			List<Map<String, Object>> orderList = orderDao.queryMerEnableMoneyByDay(inputParam.getParamString());
			if (StringUtil.listIsEmpty(orderList)) {
				queryOrderOut.setReturnCode(StringConstans.returnCode.FAIL);
				queryOrderOut.setReturnMsg("未查询到此商户当天交易,无可退款金额");
				return queryOrderOut;
			}
			...
			logger.error("查询商户当天可退款金额信息失败" + e.getMessage(), e);
			throw new FrameException(e);
		}
		logger.info("查询商户当天可退款金额返回报文:"+queryOrderOut.toString());
		return  queryOrderOut;
	}
	...
	public OutputParam updateOrderState(InputParam inputParam) throws FrameException {
		logger.info("更新订单请求报文:"+inputParam.toString());
		OutputParam  updateOrderOut = new OutputParam();
		...
		logger.info("更新订单返回报文:"+updateOrderOut.toString());
		return updateOrderOut;
	}
    ...
    public OutputParam updateWxOrderInfo(InputParam input) throws FrameException {
        ...
        try {
            ...
            if (!StringUtil.isNull(input.getValue("txnSta"))) {
                orderParam.put("txnSta", input.getValue("txnSta").toString().trim());
            }
            
            if (!StringUtil.isNull(input.getValue("txnDt"))) {
                orderParam.put("txnDt", input.getValue("txnDt").toString().trim());
            }
            
            if (!StringUtil.isNull(input.getValue("txnTm"))) {
                orderParam.put("txnTm", input.getValue("txnTm").toString().trim());
            }
            ...
            //已退款金额
            if (!StringUtil.isNull(input.getValue(Dict.totalRefundFee))) {
            	orderParam.put(Dict.totalRefundFee, input.getValue(Dict.totalRefundFee).toString().trim());
            }
            ...
            if (!StringUtil.isNull(input.getValue(Dict.initTotalRefundFee))) {
            	orderParam.put(Dict.initTotalRefundFee, input.getValue(Dict.initTotalRefundFee).toString().trim());
            }
