...
import com.huateng.pay.manager.alipay.impl.AliPayManagerImpl;
...
public class DefaultAlipayClient implements AlipayClient {
	private static final Logger logger = LoggerFactory.getLogger(DefaultAlipayClient.class);
    ...
    private <T extends AlipayResponse> RequestParametersHolder getRequestHolderWithSign(AlipayRequest<?> request,
        ...
        String app = Constants.getParam("appid");
        logger.info("配置文件中appid:"+app);
        if(this.appId.equals(app) ){
        	this.sign_type = "RSA";
        } else {
        	this.sign_type = "RSA2";
        }
        logger.info("签名算法:" + this.sign_type);
