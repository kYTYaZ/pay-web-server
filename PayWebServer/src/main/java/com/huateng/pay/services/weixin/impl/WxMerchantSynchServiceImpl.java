...
public class WxMerchantSynchServiceImpl implements WxMerchantSynchService {
	...
	public OutputParam routing(InputParam input) {
		...
		String subMerchant = StringUtil.toString(input.getValue(Dict.subMchId));
		String subWxMerId = StringUtil.toString(input.getValue(Dict.subWxMerId));
		if(StringUtil.isEmpty(subMerchant)) {
			subMerchant = subWxMerId;
		}
		
		InputParam inputQuery = new InputParam();
		inputQuery.putparamString(Dict.merId, merId);
		inputQuery.putparamString(Dict.subMerchant, subMerchant);
		...
		if (!StringConstans.returnCode.SUCCESS.equals(outQuery.getReturnCode())) {
			...
			out.putValue(Dict.respCode, outQuery.getValue(Dict.respCode));
			out.putValue(Dict.respDesc, outQuery.getValue(Dict.respDesc));
			return out;
		}
		...
		out.putValue(Dict.rate, rateChannel);
		input.putParams(Dict.rate, rateChannel);
		input.putParams(Dict.sub_mch_id, subMerchant);
		return out;
	}
...
//		requestMap.put(Dict.pfxPwd, pwd);
	...
	public OutputParam queryWxMerYL(InputParam input) throws FrameException {
		...
		logger.info("[微信断直连同步商户查询 ]  queryWxMer START:" + input.toString());
		...
		try {
			...
			if (StringUtil.isEmpty(subMchId)) {
				outputParam.setReturnCode(StringConstans.returnCode.FAIL);
				outputParam.setReturnMsg("[微信断直连同步商户查询 ]  请求报文字段[subMchId]不能为空");
				return outputParam;
			}
			...
			String returnCode = String.format("%s", respMap.get("return_code"));
			if (!Dict.SUCCESS.equals(returnCode)) {
				outputParam.putValue(Dict.respCode, StringConstans.RespCode.RESP_CODE_03);
				outputParam.putValue(Dict.respDesc, "查询微信商户失败");
			} else {
				List<Map<String, Object>> list= JsonUtil.parseJSON2List(respMap.get(Dict.mchinfo));
				if(list!=null&&list.size()==1) {
					Map<String, Object> mchinfoMap =list.get(0);
					
					outputParam.putValue(Dict.respCode, StringConstans.RespCode.RESP_CODE_02);
					outputParam.putValue(Dict.respDesc, "查询微信商户成功");
					outputParam.putValue(Dict.mchtName, mchinfoMap.get(Dict.merchant_name));
					outputParam.putValue(Dict.mchtShortName, mchinfoMap.get(Dict.merchant_shortname));
					outputParam.putValue(Dict.servicePhone, mchinfoMap.get(Dict.service_phone));
					outputParam.putValue(Dict.contactName, mchinfoMap.get(Dict.contact));
					outputParam.putValue(Dict.contactPhone, mchinfoMap.get(Dict.contact_phone));
					outputParam.putValue(Dict.contactEmail, mchinfoMap.get(Dict.contact_email));
					outputParam.putValue(Dict.business, mchinfoMap.get(Dict.business));
				}
				
				outputParam.putValue(Dict.total, respMap.get(Dict.total));
			}
		} catch (Exception e) {
